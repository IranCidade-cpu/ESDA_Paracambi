...
int main(void) {
    ...
    const char *shopping_items[INITIAL_SHOP_ITEMS]; // CORREÇÃO: const char*
    ...
    for (int i = 0; i < INITIAL_SHOP_ITEMS; i++) {
        ...
        shopping_items[i] = example_items[index];
    ...
    // Inicializar todas as posições do mundo
    for (int i = 0; i < world_count; i++) {
        world[i].collected = 0;
    }

    // Distribuir os itens da lista aleatoriamente no mundo
    // CORREÇÃO: Estratégia simplificada para posicionar itens
    int positions_used[MAP_SIZE * MAP_SIZE] = {0};
    
    // Primeiro, posicionar os itens da lista de compras
    for (int i = 0; i < INITIAL_SHOP_ITEMS; i++) {
        strncpy(world[i].name, shopping_items[i], MAX_NAME-1);
        world[i].name[MAX_NAME-1] = '\0';
    }

    // Depois, preencher os itens extras
    for (int i = INITIAL_SHOP_ITEMS; i < world_count; i++) {
    ...
    // Posicionar itens no mapa de forma aleatória
    for (int i = 0; i < world_count; i++) {
        int x, y;
        do {
            x = rand() % MAP_SIZE;
    ...
    while (1) {
        ...
        // Marcar itens coletados com 'X'
        for (int i = 0; i < world_count; i++) {
            if (world[i].collected) {
                map[world[i].y][world[i].x] = 'X';
            }
        }
